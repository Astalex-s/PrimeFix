...
- Сервис бэкенда (FastAPI, заявки по API `/api/leads/`)
- Фронтенд (одностраничный сайт с формой заявки, собирается в `frontend/dist`)
...
Будут запущены:
- `nginx` (доступен на порту `80`) — отдаёт фронтенд и проксирует `/api/` на бэкенд
- `registry` (доступен на порту `5000`)
- `pgadmin` (доступен на порту `5050`)
- `postgres` (без проброса портов на хост)
- `watchtower`
- `backend` (API, доступ только через Nginx)

Сайт открывается по адресу **http://31.128.45.161** (по IP) и по домену **http://autonode.ru**, если он настроен (см. ниже). Страница администраторов: **http://autonode.ru/admin** (вход и авторизация через `admin_settings` будут добавлены позже).

### Доступ по домену autonode.ru

Чтобы сайт открывался по **autonode.ru**, настройте DNS у регистратора домена:

| Тип  | Имя  | Значение      | TTL (по желанию) |
|------|------|---------------|------------------|
| A    | @    | 31.128.45.161 | 300–3600         |
| A    | www  | 31.128.45.161 | 300–3600         |

После сохранения подождите 5–30 минут (распространение DNS). Проверка: `ping autonode.ru` или открыть в браузере **http://autonode.ru**.

Убедитесь, что на сервере **31.128.45.161** порт 80 открыт (фаервол/облако).

### Сборка и обновление фронтенда

Фронтенд собирается **внутри образа Nginx** при `docker compose build`, поэтому сайт открывается сразу после `docker compose up -d` (или `docker compose up -d --build` при первом запуске). Ручной запуск `npm run build` на хосте не нужен.

Чтобы обновить сайт после изменений в каталоге `frontend/`:
...
Услуги хранятся в таблице **`services`**: каждая услуга — отдельная строка с полями `name` (название) и `description` (описание). При выборе услуги в форме под полем показывается описание.

- **Админ добавляет услуги по одной** через API:
  - `GET /api/admin/services/` — список услуг
  - `POST /api/admin/services/` — добавить: `{"name": "Консультация", "description": "Разовая консультация по проекту"}`
  - `PATCH /api/admin/services/{id}` — изменить

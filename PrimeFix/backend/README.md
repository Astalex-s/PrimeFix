...
backend/
  ...
  behavior_metrics/   # Поведенческие метрики (трекинг, хитмэп)
...
Трекер на главной странице каждую секунду отправляет данные о поведении пользователя.

POST и PATCH — публичные (используются трекером). GET и DELETE — защищены JWT.

#### POST `/api/behavior-metrics/` — Создание сессии метрик
...
```json
{
  ...
  "application_id": 0,
  "time_on_page": 5,
  "buttons_clicked": "{\"Оформить заказ\":2}",
  "cursor_positions": "{\"w\":1920,\"h\":5000,\"pts\":[[100,200],[300,400]]}",
  "return_frequency": 0,
  "created_at": "2026-02-09T12:00:00Z",
...
#### PATCH `/api/behavior-metrics/{metrics_id}` — Обновление метрик сессии

Вызывается трекером каждую секунду для обновления накопленных данных.

**200 OK:** обновлённый объект `BehaviorMetrics`.

**404 Not Found:**
```json
{"detail": "Behavior metrics not found"}
```

---

#### GET `/api/behavior-metrics/` — Список сессий (требуется JWT)

Параметры: `skip`, `limit`.

**200 OK:** массив объектов `BehaviorMetrics`.
...
#### GET `/api/behavior-metrics/{metrics_id}` — Сессия по ID (требуется JWT)

**200 OK:** объект `BehaviorMetrics`.
...
#### DELETE `/api/behavior-metrics/{metrics_id}` — Удаление сессии (требуется JWT)

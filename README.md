...
Ссылка на телеграм канал: https://t.me/portfolio_Alexey_Astafyev/28
...
| Порт | Сервис | Доступ |
|------|--------|--------|
| **80** | Nginx | Открыт снаружи. Сайт: `http://<хост>:80/`. Админка: `http://<хост>/admin/`. API: `http://<хост>/api/`. |
| 5050 | pgAdmin | По умолчанию **отключён**. Раскомментировать в `docker-compose.yml` → `http://<хост>:5050`. |
| 5000 | Docker Registry | По умолчанию **отключён**. Раскомментировать в `docker-compose.yml` → пуш/пул образов на `http://<хост>:5000`. |

Сервисы **backend** (FastAPI) и **postgres** портов на хост не пробрасывают — доступны только внутри Docker-сети. Обращение к API и к БД идёт через Nginx (порт 80) и из контейнеров соответственно.
